...
func buildSkillPrompt(text string, request SkillClassificationRequest) string {
	...
	return fmt.Sprintf(
		"Choose one skill inside the already selected group.\n"+
			"Return JSON only:\n"+
			"{\"skill\":\"string\",\"arguments\":{\"service\":\"\",\"text\":\"\",\"id\":\"\"},\"confidence\":0.0,\"needs_clarification\":false,\"clarification_question\":\"\"}\n\n"+
			"Available skills:\n%s\n"+
			"%s\n"+
			"Rules:\n"+
			"- skill must be one of the listed skills\n"+
			"- use empty skill only when needs_clarification=true\n"+
			"- arguments.service/text/id must always exist; use \"\" when unused\n"+
			...
			"- if needs_clarification=true then ask one short question\n\n"+
			"Examples:\n"+
			"\"сколько памяти занято\" -> {\"skill\":\"memory\",\"arguments\":{\"service\":\"\",\"text\":\"\",\"id\":\"\"},\"confidence\":0.95,\"needs_clarification\":false,\"clarification_question\":\"\"}\n"+
			"\"покажи логи tailscale\" -> {\"skill\":\"service_logs\",\"arguments\":{\"service\":\"tailscale\",\"text\":\"\",\"id\":\"\"},\"confidence\":0.95,\"needs_clarification\":false,\"clarification_question\":\"\"}\n"+
			"\"удали заметку 3\" -> {\"skill\":\"note_delete\",\"arguments\":{\"service\":\"\",\"text\":\"\",\"id\":\"3\"},\"confidence\":0.95,\"needs_clarification\":false,\"clarification_question\":\"\"}\n"+
			"\"сделай что-нибудь с интернетом\" -> {\"skill\":\"\",\"arguments\":{\"service\":\"\",\"text\":\"\",\"id\":\"\"},\"confidence\":0.40,\"needs_clarification\":true,\"clarification_question\":\"Check status, logs, or restart a service?\"}\n\n"+
			"User: %q\n",
		availableSkills,
		allowedServicesSection,
		text,
